...
# It seems that Azuracast actually needs MariaDB 10.9? Actual default Ubuntu version is 10.6
...
if [ "$azuracast_git_version" = "stable" ]; then
    ...
    | sudo bash -s -- --mariadb-server-version="mariadb-$set_mariadb_version"
else
    apt_get_with_lock install -y software-properties-common gnupg2 curl
    
    mkdir -p /etc/apt/keyrings
    
    curl -fsSL https://mariadb.org/mariadb_release_signing_key.asc \
    | gpg --dearmor | tee /etc/apt/keyrings/mariadb.gpg >/dev/null
    
    # Legacy fallback for apt-key (required for MaxScale repo)
    apt-key add /etc/apt/keyrings/mariadb.gpg
    apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys \
    5D87FACA8C27D14E CE1A3DD5E3C94F49
...
# Because of AzuraCast Supervisor Integration
